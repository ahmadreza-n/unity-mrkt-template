...
// Modified for Ar/VR TTS with callbacks
...
{
  /**
   * This was a pain to implement and extremely difficult to debug.
   * Do not look to close into the implementation, I focused to get a MVP that worked and implemented the actual logic in UWPTextToSpeech.
   */
  ...
  [AddComponentMenu("Scripts/tts/TextToSpeechImp")]
  public class TextToSpeechImp : MonoBehaviour
  {
    ...
    private static float[] ToUnityAudio(byte[] wavAudio, out int sampleCount, out int frequency)
    {
      ...
      int channelCount =
          wavAudio[22]; // Speech audio data is always mono but read actual header value for processing
      ...
      while (!(wavAudio[pos] == 100 && wavAudio[pos + 1] == 97 && wavAudio[pos + 2] == 116 &&
               wavAudio[pos + 3] == 97))
      {
        pos += 4;
        int chunkSize = wavAudio[pos] + wavAudio[pos + 1] * 256 + wavAudio[pos + 2] * 65536 +
                        wavAudio[pos + 3] * 16777216;
        pos += 4 + chunkSize;
      }
    ...
    // #define WINDOWS_UWP
        ...
        /// </param>
        private Tuple<AudioSource, AudioClip, List<TtsTrack>> PlaySpeech(string text, Func<IAsyncOperation<SpeechSynthesisStream>> speakFunc)
        {
                ...
                try
                {
                    ...
                    if (voice != TextToSpeechVoice.Default)
                    {
                        ...
                        if ((voiceInfo == null) || (!voiceInfo.DisplayName.Contains(voiceName)))
                        {
                            ...
                            voiceInfo = SpeechSynthesizer.AllVoices.Where(v => v.DisplayName.Contains(voiceName))
                                .FirstOrDefault();
                    ...
                    // SpeechSynthesisStream speechStream = await speakFunc();
                    
                    var task = Task.Run<SpeechSynthesisStream>(async () => await speakFunc());
                    SpeechSynthesisStream speechStream = task.Result;

                    // Get the size of the original stream
                    var size = speechStream.Size;
                    // Dictionary<string, Dictionary<string, object>[]> timingTracks = new Dictionary<string, Dictionary<string, object>[]>();

                    // Iterate tracks and cues and populate the tracks array
                    List<TtsTrack> tracksRef = new List<TtsTrack>();
          
                    var timedTracks = speechStream.TimedMetadataTracks;
                    // I know this could be cleaner but IDC, its a pain to write code for here and I want to skip debugging.
                    foreach (var track in timedTracks)
                    {
                        // https://docs.microsoft.com/en-us/uwp/api/windows.media.core.timedmetadatatrack?view=winrt-22000
                        List<TtsCue> ttsCues = new List<TtsCue>();
                        foreach (var cue in track.Cues)
                        {
                            // https://docs.microsoft.com/en-us/uwp/api/windows.media.core.imediacue?view=winrt-22000
                            // We assume these are all of type SpeechCue
                            // https://docs.microsoft.com/en-us/uwp/api/windows.media.core.speechcue?view=winrt-22000
                            var cue_cast = (SpeechCue) cue;
                            ttsCues.Add(new TtsCue(
                                cue_cast.Duration, cue_cast.EndPositionInInput, cue_cast.Id,
                                cue_cast.StartPositionInInput, cue_cast.StartTime, cue_cast.Text
                            ));
                        }


                        TtsTrack ttsTrack = new TtsTrack((TrackType) Enum.Parse(typeof(TrackType), track.Id), ttsCues);
                        tracksRef.Add(ttsTrack);
                    }
                    ...
                    using (var inputStream = speechStream.GetInputStreamAt(0))
                    {
                        ...
                        using (var dataReader = new DataReader(inputStream))
                        {
                            ...
                            dataReader.LoadAsync((uint) size);
                    ...
                    return Tuple.Create(audioSource, clip, tracksRef);
    ...
    public Tuple<AudioSource, AudioClip, List<TtsTrack>> SpeakSsml(string ssml)
    {
      // Make sure there's something to speak
      ...
      // Pass to helper method
            ...
            return PlaySpeech(ssml, () => synthesizer.SynthesizeSsmlToStreamAsync(ssml));
#else
